...
```bash
python .Codex/skills/optimize-images/optimize.py <path> --dry-run --json
```
...
```bash
python .Codex/skills/optimize-images/optimize.py <path>
```
...
```bash
python .Codex/skills/optimize-images/optimize.py <path> --apply-html
```
...
אחרי שה־SKILL מאומת ויציב, נוסיף feedback memory rule שיגרום ל־Codex להציע אוטומטית להריץ `/optimize-images` כשהמשתמש מוסיף תמונות חדשות לדף נחיתה (לפני commit).
